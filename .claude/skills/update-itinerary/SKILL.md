...
3. If `data.js` changed, bump its cache-busting version in `index.html` — find `<script src="data.js?v=N">` and increment `N`. Leave `app.js`'s `?v=` alone unless you also touched `app.js` (which this skill normally shouldn't). Also update `TRIP.updated` at the top of `data.js` (an ISO timestamp, e.g. `2026-08-15T05:42:57Z`) to the current UTC time — it drives the "Updated ... ago" line at the top of the site, so it should reflect this edit.
